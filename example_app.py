...
class ExampleApp(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def changeGridCellSize(self):
        ...
        dialog.show()
        if dialog.exec_():
            # Изменяем размер сетки для активного изображения
            self.appState.activeGrid.changeCellSize(dialog.gridCellSize)
            if dialog.applyToAllGrids:
                # Создаем прогресс-диалог для изменения размеров всех сеток
                progressDialog = QtWidgets.QProgressDialog("Изменение размера сеток...", "Отмена", 0,
                                                           self.appState.activeFolder.totalFilesCount, self)
                progressDialog.setWindowModality(QtCore.Qt.WindowModal)
                progressDialog.setMinimumDuration(0)
                progressDialog.setValue(0)
                # Подключаем сигнал обновления прогресса
    ...
    def createGrid(self):
        ...
        dialog.show()
        if dialog.exec_():
            # Устанавливаем новую сетку для активного изображения
            self.appState.setActiveGrid(ClassGrid(None, None, dialog.gridCellSize, dialog.gridSize))
            if dialog.applyToAllGrids:
                progressDialog = QtWidgets.QProgressDialog("Создание сеток...", "Отмена", 0,
                                                           self.appState.activeFolder.totalFilesCount, self)
                progressDialog.setWindowModality(QtCore.Qt.WindowModal)
                progressDialog.setMinimumDuration(0)
                progressDialog.setValue(0)
    ...
    def setTool(self, tool):
        ...
        # print("Active tool set to:", tool.name()) я тебя в рот ебал ебанный принт
